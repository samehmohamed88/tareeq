...
//    LibUsbDevice();
    ...
    int libUsbControlTransfer(uint8_t bmRequestType,
                                               uint8_t bRequest,
                                               uint16_t wValue,
                                               uint16_t wIndex,
                                               uint8_t *data,
    ...
    /// If the transfer times out, the function will return `LIBUSB_ERROR_TIMEOUT`.
    /// @returns libusb_error a status of either success or timeout, etc.
    int libUsbBulkTransfer(uint8_t endpoint,
                                            uint8_t *data,
                                            int length,
                                            int *actual_length,
                                            unsigned int timeout) const override;
    ...
    /// @returns libusb_error a status of either success or timeout, etc.
    libusb_error libUsbInitDevice() override;
    ...
    /// @returns libusb_error a status of either success or timeout, etc.
    libusb_error libUsbOpenDevice(uint16_t vendorID, uint16_t productID) override;
    ...
    /// @returns libusb_error a status of either success or timeout, etc.
    libusb_error libUsbSetDefaultConfiguration() const override;
    ...
    /// @returns libusb_error a status of either success or timeout, etc.
    libusb_error libUsbDetachKernelDriver(int interfaceNumber) const override;

    /// Calls the underlying `libusb_claim_interface`.
    /// @param interfaceNumber : The number of the interface for which the kernel driver should be detached.
    /// This is typically the interface you intend to communicate with.
    /// @returns libusb_error a status of either success or timeout, etc.
    libusb_error libUsbClaimInterface(int interfaceNumber) const override;
