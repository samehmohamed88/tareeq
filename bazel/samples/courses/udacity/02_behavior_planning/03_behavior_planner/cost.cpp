                         ...
                         map<string, float>& data)
{
    ...
    // Cost of being out of goal lane also becomes larger as vehicles approaches
    //   goal distance.
                        ...
                        map<string, float>& data)
{
    ...
    //   that have traffic slower than vehicles's target speed.
...
float lane_speed(const map<int, vector<Vehicle>>& predictions, int lane)
{
    ...
    //   limit for a lane, we can just find one vehicles in that lane.
                     ...
                     const vector<Vehicle>& trajectory)
{
    ...
    vector<std::function<float(
        const Vehicle&, const vector<Vehicle>&, const map<int, vector<Vehicle>>&, map<string, float>&)>>
                                   ...
                                   const map<int, vector<Vehicle>>& predictions)
{
    ...
    // intended_lane: the current lane +/- 1 if vehicles is planning or
    //   executing a lane change.
    // final_lane: the lane of the vehicles at the end of the trajectory.
    // distance_to_goal: the distance of the vehicles to the goal.
