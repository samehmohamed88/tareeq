...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>>& predictions)
{
     ...
     *
     * @param A predictions map. This is a map of vehicles id keys with predicted
     *   vehicles trajectories as values. Trajectories are a vector of Vehicle
     *   objects representing the vehicles at the current timestep and one timestep
     *   in the future.
     ...
     *    a vehicles trajectory, given a state and predictions. Note that
...
//     std::string next_state;
    ...
    for (const auto& possible_state: successor_states) {
         auto trajectory = generate_trajectory(possible_state, predictions);
        Vehicle v = *this;
...
vector<float> Vehicle::get_kinematics(map<int, vector<Vehicle>>& predictions, int lane)
{
    ...
    //   given other vehicles positions and accel/velocity constraints.
...
vector<Vehicle> Vehicle::lane_change_trajectory(string state, map<int, vector<Vehicle>>& predictions)
{
    ...
    // Check if a lane change is possible (check if another vehicles occupies
    //   that spot).
...
{
    // Returns a true if a vehicles is found behind the current vehicles, false
    //   otherwise. The passed reference rVehicle is updated if a vehicles is found.
    int max_s = -1;
...
{
    // Returns a true if a vehicles is found ahead of the current vehicles, false
    //   otherwise. The passed reference rVehicle is updated if a vehicles is found.
...
vector<Vehicle> Vehicle::generate_predictions(int horizon)
{
    ...
    //   generation for the ego vehicles.
...
{
    // Sets state and kinematics for ego vehicles using the last state of the trajectory.
