...
void VehicleDynamicModel::load_vehicle_data(
  ...
  // Get some basic Physical data from the vehicles
...
AD<double> VehicleDynamicModel::get_accel(
  ...
  {
    // getting the deceleration from the vehicles's brake model
    return get_decel_brake(throttle, velocity, pitch);
  } else {
    // getting the acceleration from the vehicles's engine model
